...
export default function Portfolio() {
  ...
  return (
    <section id="portfolio" className="py-24 max-w-7xl mx-auto px-6 relative overflow-hidden select-none">
      ...
      <div className="absolute top-[20%] left-[5%] w-[200px] h-[200px] rounded-full bg-[#2563EB]/4 blur-[80px] pointer-events-none" />
      <div className="absolute bottom-[20%] right-[5%] w-[250px] h-[250px] rounded-full bg-[#1D4ED8]/3 blur-[100px] pointer-events-none" />

      {/* 👑 SECTION HEADER */}
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-6 mb-16 relative z-10">
        <div className="text-right flex flex-col gap-3">
          <span className="px-4 py-1.5 rounded-full border border-blue-500/30 bg-blue-500/8 text-[#60A5FA] font-medium text-[10px] tracking-widest uppercase w-fit">
            ✦ معرض أعمالنا
          </span>
          <h2 className="font-marcellus text-3xl md:text-5xl font-semibold text-white leading-tight">
            أعمال نفخر بصياغتها
          </h2>
          <p className="max-w-xl text-[#94A3B8] font-normal text-xs sm:text-sm leading-relaxed">
              ...
              >
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-gradient-to-r from-blue-600 to-blue-500 rounded-full border border-blue-500/25 shadow-md shadow-blue-500/10"
        ...
        <AnimatePresence mode="popLayout">
          {filteredProjects.map((proj) => (
            <motion.div
              ...
              className={`rounded-[28px] overflow-hidden border border-blue-500/10 bg-[#030B1A]/80 backdrop-blur-md shadow-soft hover:shadow-[0_0_30px_rgba(37,99,235,0.15)] group flex flex-col justify-between h-full cursor-pointer min-h-[440px] md:min-h-[500px] ${proj.colSpan}`}
              onClick={() => setSelectedProject(proj)}
            >
              {/* Image Preview Container (Zoom Reveal & Soft Gradients) */}
              <div className="w-full aspect-[16/10] relative overflow-hidden select-none border-b border-blue-500/10 bg-[#020817] flex items-center justify-center p-3 group-hover:from-blue-950/20 group-hover:to-blue-950/40 transition-all duration-700">
                <img 
                  loading="lazy"
                  src={proj.image} 
                  className="w-full h-full object-contain object-center group-hover:scale-[1.03] transition-transform duration-700 ease-out drop-shadow-[0_6px_16px_rgba(0,0,0,0.25)]" 
                  alt={proj.title} 
                />
 
                {/* Hover direct action trigger */}
                <div className="absolute inset-0 bg-[#020817]/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <motion.div 
                    whileHover={{ scale: 1.05 }}
                    className="px-6 py-3 rounded-full bg-[#030B1A] text-white text-xs font-medium shadow-premium border border-blue-500/20 flex items-center gap-1.5"
                  >
              ...
              <div className="p-7 flex flex-col justify-between flex-grow bg-[#030B1A]/40 backdrop-blur-sm">
                <div className="flex flex-col gap-2">
                  <span className="text-[9px] text-[#60A5FA] font-medium uppercase tracking-widest">
                    {categories.find(c => c.id === proj.category)?.label || 'مشروع متميز'}
                  </span>
 
                  <h3 className="font-marcellus text-xl font-semibold text-white transition-colors group-hover:text-[#60A5FA]">
                    {proj.title}
                  </h3>
 
                  <p className="text-xs text-[#94A3B8] font-normal leading-relaxed mb-4 text-right">
                    {proj.desc}
                  </p>
                </div>
 
                {/* Micro tags and arrow */}
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-blue-500/10">
                  <div className="flex flex-wrap gap-1.5">
                    {proj.tags.map((tag, idx) => (
                      <span 
                        key={idx} 
                        className="px-2.5 py-1 rounded-full bg-[#020817] border border-blue-500/10 text-[9px] font-medium text-[#94A3B8]"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="w-8 h-8 rounded-full bg-blue-500/10 group-hover:bg-[#2563EB] text-white flex items-center justify-center transition-all duration-500">
        ...
        {selectedProject && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-3xl bg-[#020817] rounded-[28px] overflow-hidden border border-blue-500/20 shadow-premium max-h-[90vh] overflow-y-auto no-scrollbar flex flex-col text-right"
            ...
            >
              ...
              <button
                onClick={() => setSelectedProject(null)}
                className="absolute top-4 left-4 p-2.5 rounded-full border border-blue-500/15 bg-[#030B1A]/80 text-[#94A3B8] hover:bg-blue-950 hover:text-white transition-all z-10 active:scale-95 cursor-pointer"
              ...
              <div className="w-full aspect-[16/10] relative overflow-hidden select-none border-b border-blue-500/15 bg-[#020817] flex items-center justify-center p-4">
                <img 
                  src={selectedProject.image} 
                  className="w-full h-full object-contain object-center drop-shadow-[0_8px_32px_rgba(0,0,0,0.3)]" 
              ...
              <div className="p-6 md:p-8 flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                  <h4 className="font-marcellus text-sm font-semibold text-white flex items-center gap-2">
                    <Laptop size={16} className="text-[#3B82F6]" />
                    نطاق ومفهوم المشروع:
                  </h4>
                  <p className="text-xs text-[#94A3B8] font-normal leading-relaxed pr-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2 pt-6 border-t border-blue-500/12">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-wrap gap-1.5 pr-6">
                      {selectedProject.details.techs.map((tech, i) => (
                        <span key={i} className="px-3 py-1.5 bg-[#030B1A] border border-blue-500/10 text-[10px] font-medium rounded-lg text-white shadow-soft">
                          {tech}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-2 pr-6">
                      {selectedProject.details.metrics.map((m, i) => (
                        <div key={i} className="flex items-center gap-2 text-xs font-semibold text-emerald-400">
                ...
                <div className="flex items-center gap-3 border-t border-blue-500/15 pt-6 mt-6">
                  <button
                    ...
                    className="w-full sm:w-auto py-3.5 px-8 bg-gradient-to-r from-blue-600 to-blue-500 text-white rounded-xl text-xs font-medium shadow-sm hover:shadow-[0_0_20px_rgba(37,99,235,0.35)] border border-blue-500/20 text-center hover:scale-[1.02] active:scale-95 transition-all duration-500 cursor-pointer"
                  >
